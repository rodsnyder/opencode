...
describe("opencode run (non-interactive subprocess)", () => {
  ...
  // The bound is the harness default (30s), not tighter: on GitHub-hosted
  // ubuntu-latest the cold `bun src/index.ts` spawn under concurrent siblings
  // alone exceeded a 15s bound, which is CLI startup cost, not the hang.
  cliIt.concurrent(
